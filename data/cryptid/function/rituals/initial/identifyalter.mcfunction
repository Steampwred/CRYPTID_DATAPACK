...
execute anchored feet positioned ~ ~-2 ~ store success storage cryptid:ritual alter.revival int 1 if predicate cryptid:alter/revival
...
#$tellraw @a "summoning new $(type) alter"



## enchanting table translate
execute as @s[tag=ritual.translate] run tellraw @a[distance=..10] {"text":"Ritual Stability: ","color":"dark_purple","bold":true,"extra":[{"score":{"name":"@s","objective":"cryptid.ritual.stability"},"color":"aqua","bold":false}]}